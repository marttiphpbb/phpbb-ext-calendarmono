...
class event_container
{
	...
	public function fetch()
	{
		...
		$sql = 'SELECT t.topic_id, t.forum_id, t.topic_reported, t.topic_title,
			...
			WHERE ( t.' . cnst::COLUMN_START . ' <= ' . $this->timespan->get_end() . '
				AND t.' . cnst::COLUMN_END . ' >= ' . $this->timespan->get_start() . ' )
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			$timespan = new timespan($row[cnst::COLUMN_START], $row[cnst::COLUMN_END]);
